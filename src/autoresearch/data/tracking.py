...
class DatasetTracker:
    ...
    def log_run(self, dataset, metrics, params, model, profile):
    ...
    def log_research_iteration(self, parent_run_id, result, drift):
        ...
        with mlflow.start_run(run_name=f"research-iteration-{result['iteration']}") as run:
            ...
            mlflow.log_params({
                ...
                "training_seed": result["training_seed"], "selected_model": result["recommended_model"],
            ...
            metrics.update({f"holdout_candidate_{key}": value for key, value in result["recommended_metrics"].items() if value is not None})
